...
std::string genericVerbsRespond(const std::string& input) {
    ...
    if (std::regex_search(input, match, dontHave)) {
        ...
        return text_utils::fillTemplate("What would it mean to you to have OBJECT?", "OBJECT",
                                         object);
    }
    ...
    if (std::regex_search(input, match, have)) {
        std::string object = text_utils::reflect(match[1].str());
        return text_utils::fillTemplate("How long have you had OBJECT?", "OBJECT", object);
    }
    ...
    if (std::regex_search(input, match, wantToPurchase)) {
        std::string object = text_utils::reflect(match[1].str());
        return text_utils::fillTemplate("Why do you want to purchase OBJECT?", "OBJECT", object);
    }
    ...
    if (std::regex_search(input, match, purchased)) {
        std::string object = text_utils::reflect(match[1].str());
        return text_utils::fillTemplate("What made you decide to purchase OBJECT?", "OBJECT",
                                         object);
    }
    ...
    if (std::regex_search(input, match, traveled)) {
        ...
        return text_utils::fillTemplate("What was your trip to PLACE like?", "PLACE", place);
    }
    ...
    if (std::regex_search(input, match, learn)) {
        ...
        return text_utils::fillTemplate("What draws you to learning about TOPIC?", "TOPIC",
                                         topic);
    }
    ...
    if (std::regex_search(input, match, know)) {
        std::string topic = text_utils::reflect(match[1].str());
        return text_utils::fillTemplate("How do you know TOPIC?", "TOPIC", topic);
    }
